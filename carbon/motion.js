...
function createPreloaderAnimation() {
  ...
  if (!allElementsFound) {
    console.error("Some elements not found, animation may not work correctly");
  }
...
// Initialize immediately and add a fallback
initPreloader();

// Also set a backup initialization with small delay
if (document.readyState !== "complete") {
  console.log("Document not fully loaded, adding fallback initialization");
  window.addEventListener("load", () => {
    console.log("Window load event - checking if preloader already ran");
    if (!window.preloaderAnimation) {
      console.log("Preloader hasn't run yet, initializing from fallback");
      initPreloader();
    }
  });
}
