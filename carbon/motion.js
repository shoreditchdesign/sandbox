...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Selectors
  const selectors = {
    wrapper: '[data-countup-el="wrap"]',
    digit: '[data-countup-el="digit"]',
    digitItem: "div",
    attributes: {
      value: "data-countup-value",
      delay: "data-countup-delay",
      digitPosition: "data-digit-value",
    },
  };

  // Stores all initialized instances
  const instances = [];

  // Initialize all countups on the page
  function initCountups() {
    console.log("Initializing all countup instances");

    const wrappers = document.querySelectorAll(selectors.wrapper);

    if (wrappers.length === 0) {
      console.error("No countup elements found on page");
      return;
    }

    console.log(`Found ${wrappers.length} countup instances`);

    wrappers.forEach((wrapper) => {
      // Get delay for this instance (or use default)
      const delayAttr = wrapper.getAttribute(selectors.attributes.delay);
      const delay = delayAttr ? parseFloat(delayAttr) : ANIMATION.defaultDelay;

      // Initialize this instance with its specific delay
      gsap.delayedCall(delay, () => initSingleCountup(wrapper));
      console.log(`Scheduled countup animation with ${delay}s delay`);
    });
  }

  // Initialize a single countup instance
  function initSingleCountup(wrapper) {
    const targetValue = wrapper.getAttribute(selectors.attributes.value);

    if (!targetValue) {
      console.error(
        `Countup wrapper missing ${selectors.attributes.value} attribute`,
      );
      return;
    }

    console.log(`Initializing countup to target: ${targetValue}`);

    const digitElements = wrapper.querySelectorAll(selectors.digit);

    if (digitElements.length === 0) {
      console.error("No digit elements found in countup wrapper");
      return;
    }
    ...
    // Initialize vertical loops for each digit
    createDigitWheels(instance);

    // Store the instance
    instances.push(instance);

    // Start animation
    animateCountup(instance);
  }

  // Create wheel animations for all digits in an instance
  function createDigitWheels(instance) {
    console.log(
      `Creating wheel animations for ${instance.digitElements.length} digits`,
    );

    instance.digitElements.forEach((digitEl) => {
      const position = digitEl.getAttribute(selectors.attributes.digitPosition);

      if (!position) {
        console.error(
          `Digit element missing ${selectors.attributes.digitPosition} attribute`,
        );
        return;
      }

      // Create path to digit items selector
      const digitItemsSelector = `${selectors.digitItem}`;
      const wheelItems = digitEl.querySelectorAll(digitItemsSelector);

      if (wheelItems.length === 0) {
        console.error(`No digit items found for position ${position}`);
        return;
      }

      // Create wheel and store with position reference
      const wheel = verticalLoop(wheelItems, { repeat: -1, paused: true });
      instance.wheels.push({
        position: parseInt(position),
        wheel,
      });
    });

    // Sort wheels by position (4,3,2,1 to ensure correct animation order)
    instance.wheels.sort((a, b) => b.position - a.position);

    console.log(`Created ${instance.wheels.length} wheel animations`);
  }

  // Animate the countup
  function animateCountup(instance) {
    const targetNumString = instance.targetValue.toString();
    const numDigits = targetNumString.length;

    console.log(`Starting animation for target ${instance.targetValue}`);

    const tl = gsap.timeline({
      onStart: () => console.log("Starting countup animation"),
      onComplete: () => console.log("Countup animation complete"),
    });

    // Animate each wheel with a slight delay, imitating an odometer
    instance.wheels.forEach((wheelObj, i) => {
      const position = wheelObj.position;
      const wheel = wheelObj.wheel;

      // Map the position to the correct digit in the target value
      // Position 1 = rightmost digit (ones place)
      const digitIndex = numDigits - position;

      // Only animate if this position exists in our target number
      if (digitIndex >= 0) {
        const targetDigit = parseInt(targetNumString[digitIndex]);

        console.log(`Position ${position}: targeting digit ${targetDigit}`);

        tl.to(
          wheel,
          {
            duration: ANIMATION.digit.duration,
            ease: ANIMATION.digit.ease,
            onStart: () =>
              wheel.toIndex(targetDigit, {
                duration: ANIMATION.digit.duration,
                revolutions: position, // More revolutions for higher positions
              }),
          },
          i * ANIMATION.digit.wheelSpacing, // Stagger animation start
        );
      }
    });
